...
import { Shield, Zap, Target, Play, RotateCcw, Info, Settings, Activity } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';

import { Point, Level, EnemyType, WaveEnemy, Wave, Node, Enemy, Tower, Projectile, Particle, FloatingText, TowerType } from './types';
import { CELL_SIZE, MAX_PARTICLES, INITIAL_LIVES, ENEMY_TYPES, LEVELS, WAVES, TOWER_TYPES } from './constants';
import { generateMaze } from './utils/maze';
import { getDistance, aStar } from './utils/pathfinding';
...
export default function App() {
  ...
  const [grid, setGrid] = useState<boolean[][]>(() => {
    const g = Array.from({ length: gridSize }, () => Array(gridSize).fill(false));
  ...
  useEffect(() => {
    ...
    const g = generateMaze(newLevel.gridSize, newLevel.startNode, newLevel.goalNode);
    setGrid(g);
  ...
  const calculateDistanceMap = useCallback((currentGrid: boolean[][]) => {
    const map = Array.from({ length: gridSize }, () => Array(gridSize).fill(Infinity));
    ...
    while (queue.length > 0) {
      ...
      for (const dir of dirs) {
        ...
        if (nx >= 0 && nx < gridSize && ny >= 0 && ny < gridSize && 
            !currentGrid[ny][nx] && map[ny][nx] === Infinity) {
    ...
    waveConfig.enemies.forEach((enemyGroup) => {
      for (let i = 0; i < enemyGroup.count; i++) {
        setTimeout(() => {
          ...
          const health = (30 + nextWave * 15) * type.healthMultiplier * difficultyMultiplier;
          const speed = (0.025 + Math.min(nextWave * 0.006, 0.05)) * type.speedMultiplier;
          ...
          enemiesRef.current.push({
            ...
            originalSpeed: speed,
            slowTimer: 0,
            reward: Math.floor((5 + Math.floor(nextWave / 2)) * type.rewardMultiplier)
          });
  ...
  useEffect(() => {
    ...
    const update = (time: number) => {
      ...
      floatingTextsRef.current = floatingTextsRef.current.map(t => ({
        ...t,
        pos: { x: t.pos.x, y: t.pos.y - 0.01 },
        life: t.life - deltaTime
      })).filter(t => t.life > 0);

      if (isWaveActiveRef.current || enemiesRef.current.length > 0 || particlesRef.current.length > 0 || floatingTextsRef.current.length > 0) {
          ...
          if (dist < 0.05 && cx === goalNode.x && cy === goalNode.y) {
            ...
            if (enemy.type === 'bomber') {
              ...
              towersRef.current = towersRef.current.filter(t => {
                const inRange = Math.abs(t.gridX - cx) <= 2 && Math.abs(t.gridY - cy) <= 2;
                return !inRange;
              });
              ...
              setScreenShake(20);
              const explosionParticles: Particle[] = Array.from({ length: 30 }).map(() => ({
                id: Math.random(),
              ...
              particlesRef.current = [...particlesRef.current, ...explosionParticles].slice(-MAX_PARTICLES);
            }
            return null;
          }
        ...
        if (enemiesRef.current.length === 0 && enemiesToSpawnRef.current === 0 && isWaveActiveRef.current) {
          isWaveActiveRef.current = false;
          ...
          if (tower.type === 'teleporter') {
            ...
            if (targetEnemy) {
              ...
              const newParticles: Particle[] = Array.from({ length: 15 }).map(() => ({
                id: Math.random(),
                pos: { x: targetEnemy.pos.x + 0.5, y: targetEnemy.pos.y + 0.5 },
        ...
        projectilesRef.current = projectilesRef.current.map(proj => {
          const target = enemiesRef.current.find(e => e.id === proj.target.id);
          ...
          if (dist < 0.2) {
            ...
            particlesRef.current = [...particlesRef.current, ...hitParticles].slice(-MAX_PARTICLES);
            ...
            if (target.health <= 0) {
              ...
              if (target.type === 'bomber') {
                ...
                const deathParticles: Particle[] = Array.from({ length: 12 }).map(() => ({
                  id: Math.random(),
                  pos: { x: target.pos.x + 0.5, y: target.pos.y + 0.5 },
                  vel: { x: (Math.random() - 0.5) * 0.08, y: (Math.random() - 0.5) * 0.08 },
                  life: 600,
                  maxLife: 600,
                  color: ENEMY_TYPES[target.type].color,
                  size: 3
                }));
                particlesRef.current = [...particlesRef.current, ...deathParticles].slice(-MAX_PARTICLES);
              }
        ...
        const totalRemaining = enemiesRef.current.length + enemiesToSpawnRef.current;
      ...
      if (ctx && canvas) {
        ...
        ctx.fillRect(goalNode.x * CELL_SIZE, goalNode.y * CELL_SIZE, CELL_SIZE, CELL_SIZE);
        ctx.strokeStyle = '#3b82f6';
        ctx.strokeRect(goalNode.x * CELL_SIZE, goalNode.y * CELL_SIZE, CELL_SIZE, CELL_SIZE);
        ctx.shadowBlur = 0;
        ...
        towersRef.current.forEach(t => {
          ...
          ctx.fillStyle = color;
          ctx.beginPath();
          ctx.roundRect(t.gridX * CELL_SIZE + 4, t.gridY * CELL_SIZE + 4, CELL_SIZE - 8, CELL_SIZE - 8, 6);
          ctx.fill();
          ctx.shadowBlur = 0;
          
          if (hoveredCell && hoveredCell.x === t.gridX && hoveredCell.y === t.gridY) {
            ctx.beginPath();
            ctx.arc(t.gridX * CELL_SIZE + CELL_SIZE / 2, t.gridY * CELL_SIZE + CELL_SIZE / 2, t.range * CELL_SIZE, 0, Math.PI * 2);
        ...
        particlesRef.current.forEach(p => {
          ...
          ctx.beginPath();
          ctx.arc(p.pos.x * CELL_SIZE, p.pos.y * CELL_SIZE, p.size, 0, Math.PI * 2);
          ctx.fill();
          ctx.globalAlpha = 1;
        });
        ...
        enemiesRef.current.forEach(e => {
          ...
          ctx.beginPath();
          ctx.arc(e.pos.x * CELL_SIZE + CELL_SIZE / 2, e.pos.y * CELL_SIZE + CELL_SIZE / 2, type.size, 0, Math.PI * 2);
          ctx.fill();
          ctx.shadowBlur = 0;
          ...
          ctx.fillStyle = '#333';
          ctx.fillRect(e.pos.x * CELL_SIZE + (CELL_SIZE - barWidth) / 2, e.pos.y * CELL_SIZE - 8, barWidth, 4);
          ctx.fillStyle = healthPercent > 0.5 ? '#10b981' : healthPercent > 0.2 ? '#f59e0b' : '#ef4444';
          ctx.fillRect(e.pos.x * CELL_SIZE + (CELL_SIZE - barWidth) / 2, e.pos.y * CELL_SIZE - 8, barWidth * healthPercent, 4);
        });
  ...
  }, [distanceMap, gridSize, goalNode, startNode, grid, hoveredCell, selectedTowerType, height, width]);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-[#e0e0e0] font-mono p-4 md:p-8 flex flex-col items-center overflow-x-hidden">
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8">
        {/* Game Area */}
        <motion.div 
          animate={screenShake > 0 ? { x: [0, -screenShake, screenShake, 0], y: [0, screenShake, -screenShake, 0] } : {}}
