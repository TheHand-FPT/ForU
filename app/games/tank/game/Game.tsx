...
export const Game: React.FC<{ onGameOver: (score: number) => void }> = ({
  onGameOver,
}) => {
  ...
  const update = (time: number) => {
    ...
    const currentSpeed = Math.min(
      MAX_PLAYER_SPEED,
      MIN_PLAYER_SPEED + player.score * SPEED_SCALING_FACTOR,
    );
    ...
    if (time - lastBotSpawnRef.current > BOT_SPAWN_INTERVAL) {
      ...
      if (emptyCells.length > 0) {
        ...
        const totalWeight =
          weights.black + weights.blue + weights.red + weights.yellow;
        // eslint-disable-next-line react-hooks/purity
        ...
        else if (random < weights.yellow + weights.red + weights.blue)
          type = "blue";
        else type = "black";
    ...
    bulletsRef.current.forEach((b) => {
      ...
      if (b.type === "PIERCE" || b.type === "HOLLOW_PURPLE") {
        ...
        const hitBot = botsRef.current.find(
          (bot) => Math.hypot(bot.x - b.x, bot.y - b.y) < b.radius + bot.radius,
        );
        if (b.target) {
    ...
    bombsRef.current.forEach((bomb) => {
      const botTriggered = botsRef.current.some(
        (bot) =>
          Math.hypot(bot.x - bomb.x, bot.y - bomb.y) <
          bot.radius + bomb.radius + 0.1, // Added slight buffer
      );
      if (botTriggered) {
  ...
  const draw = () => {
    ...
    // Draw Player
    const p = playerRef.current;
    ctx.fillStyle = p.activePowerUp
      ? LOOT_CONFIG[p.activePowerUp].color
      : "#000";
    ctx.beginPath();
    ctx.arc(
      p.x * CELL_SIZE,
    ...
    ctx.fillRect(
      p.x * CELL_SIZE - playerBarWidth / 2,
      (p.y - p.radius - 0.2) * CELL_SIZE,
      playerBarWidth,
      playerBarHeight,
    );
    ...
    bombsRef.current.forEach((bomb) => {
      ...
      ctx.arc(
        bomb.x * CELL_SIZE,
        (bomb.y - bomb.radius - 0.1) * CELL_SIZE,
        2,
        0,
        Math.PI * 2,
      );
    ...
    botsRef.current.forEach((bot) => {
      ...
      if (bot.maxHp > 1) {
        ...
        ctx.fillRect(
          (bot.x - bot.radius) * CELL_SIZE,
          (bot.y - bot.radius - 0.2) * CELL_SIZE,
          barWidth,
          barHeight,
        );
    ...
    lootRef.current.forEach((l) => {
      ...
      ctx.fillStyle = "#fff";
      ctx.font =
        l.type === "HOLLOW_PURPLE" ? "bold 16px serif" : "bold 12px monospace";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(
        LOOT_CONFIG[l.type].symbol,
        l.x * CELL_SIZE,
        l.y * CELL_SIZE,
      );
    });
  ...
  return (
    <div
      className={`flex flex-col items-center gap-4 w-full max-w-full transition-transform duration-75 ${gojoEffect ? "animate-shake" : ""}`}
    >
